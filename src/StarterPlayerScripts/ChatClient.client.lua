...
local function createBubble(character: Model, text: string)
	...
	billboard.MaxDistance       = 0         -- visible at any distance that received the event
	billboard.StudsOffset       = Vector3.new(0, 2.2, 0)   -- float above head
	billboard.Size              = UDim2.new(0, CFG.BUBBLE_MAX_WIDTH + CFG.BUBBLE_PADDING_H * 2, 0, 50)
	billboard.SizeOffset        = Vector2.new(0, 0.5)
	billboard.ResetOnSpawn      = false
	billboard.Adornee           = attachPart
	billboard.Parent            = character

	-- Auto-shrink to fit: we use AutomaticSize on the inner frame
	...
	label.Size                  = UDim2.new(0, CFG.BUBBLE_MAX_WIDTH, 0, 0)
